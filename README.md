...
- **Read** — LLMs have become very good at exploring filesystems with bash-style tools. We lean in: `ls`, `read`, `grep` over the project namespace are always-on meta-tools, including binary documents (PDFs, images). Same mental model as Claude Code. Content from docs the harness itself didn't write (artefacts, uploads, anything outside the canonical set) is wrapped in `<untrusted source="…">` *spotlight* tags before the LLM sees it, and the system prompt instructs the model to treat tag contents as data, not instructions — the cheapest credible mitigation for indirect prompt injection given that the agent is allowed to read freely.
- **Write** — every mutation goes through a typed `@action` declared by the implementer. No arbitrary edits. No shell escape hatch. Each call is appended to a hash-chained event log with the caller's `AgentIdentity` (principal, instance id, optional pubkey fingerprint, roles), the originating `TriggerEvent.id`, arguments, and outcome — so any post-hoc edit shows up immediately as a `ChainBreak`.
...
| `<project_id>/audit.jsonl` | One-line-per-action machine audit log with timestamps, args, outcomes, the caller's `AgentIdentity` label, optional pubkey fingerprint, and the originating `trigger_id`. Mirror of `event_log.md` for programmatic queries. | Tooling |
...
- Stamping the caller's `AgentIdentity` and the originating `TriggerEvent.id` onto every `Event`, so `event_log.md`, `audit.jsonl`, and `search_event_log` queries all attribute back to a specific principal and request chain
...
Every hook receives an `ActionContext` (or `SubagentContext`) carrying the caller's `AgentIdentity`, the originating `trigger_id`, and the per-run `RunBudget` — so policy hooks can branch on `ctx.identity.has_role("partner")`, log to an external system using the same correlation id, or tighten the remaining call budget mid-run.

**Run budgets.** Each run is bounded by a `RunBudget` (default: 50 actions, 10 subagent consults per `TriggerEvent`). The runtime checks the budget before every `@action` and `consult_<name>` call; exceeding the cap surfaces as the *same* blocking TOOL message shape as a user-defined `pre_action` block, so the LLM has the chance to explain the limit to the user rather than thrashing. Tighten the defaults at runtime construction (`AgentRuntime(..., default_budget=RunBudget(action_limit=5, subagent_limit=1))`) or mutate `ctx.budget` from a hook to apply a per-actor / per-trigger policy.
...
**Tamper-evident hash chain.** On every successful append the log fills in `Event.prev_hash` (the previous event's `hash`) and `Event.hash` (a SHA-256 over a canonical encoding of every other field, including the identity claim). `EventLog.verify_chain(project_id)` re-derives every link and raises `ChainBreak` at the first inconsistency — any post-hoc edit to the stored log, on any backend, shows up at the offset where it happened. The chain logic lives on the ABC, so Kafka, S3, or any future backend gets integrity verification for free as long as it persists the two hash fields.
...
| F3 | Every `@action` invocation must be recorded in an append-only audit log with: caller `AgentIdentity` (principal, instance id, optional pubkey fingerprint, roles), originating `TriggerEvent.id`, method name, parameters, timestamp, log offset, and outcome (result type or error) |
...
| F10 | The event log must be tamper-evident — every appended `Event` carries `prev_hash` linking it to its predecessor and `hash` covering its own canonical form; `EventLog.verify_chain(project_id)` must raise `ChainBreak` on any post-hoc edit, regardless of backend |
| F11 | Each run must enforce a `RunBudget` (per-trigger caps on `@action` and `consult_<name>` calls); exceeding a cap must surface to the LLM via the same blocking-TOOL-message shape as a user-defined `pre_action` block, so the agent can adapt rather than thrash |
...
| N5 | Content returned by the `read` / `grep` meta-tools from docs outside the canonical set (`state.json`, `summary.md`, `event_log.md`, `audit.jsonl`) must be wrapped in `<untrusted source="…">` spotlight tags, with literal closing tags inside the body escaped so the wrapper cannot be forged; the default system preamble must instruct the LLM to treat tag contents as data, not instructions |
